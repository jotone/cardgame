$(document).ready(function(){
	...
	$('button[name=savePromo]').click(function(){
		if($('input[name=title]').val().trim() == ''){
			...
			//discount
			var discount = {
				value: $('input[name=discountValue]').val(),
				type:  $('input[name=discountType]:checked').val()
			};
			...
			//Regular dates
			if($(document).find('input[name=date_begin]').length > 0){
				formData.append('date_begin', $(document).find('input[name=date_begin]').val());
			}else{
				formData.append('date_begin', '0000-00-00 00:00:00');
			}
			if($(document).find('input[name=date_finish]').length > 0){
				formData.append('date_finish', $(document).find('input[name=date_finish]').val());
			}else{
				formData.append('date_finish', '0000-00-00 00:00:00');
			}
					...
					try{
						...
						if(data['message'] == 'success'){
							if($('input[name=id]').val().length == 0){
								var redirectQuestion = confirm('Перейти к списку акций?');
								if(redirectQuestion){
									location = returnPath;
								}else{
