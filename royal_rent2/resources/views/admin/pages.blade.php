...
@stop
@section('content')
	<div class="main-block" data-sort="{{ $active_direction['sort'] }}" data-direction="{{$active_direction['dir']}}">
		<div class="center-wrap col_1">
			<div class="page-caption row-wrap">{{ $page_title }}</div>
			<div class="button-wrap">
				<a class="control-button" href="{{ URL::asset('/admin/pages/add') }}">Добавить</a>
			</div>
			@if(1 < $pagination['last_page'])
				<div class="row-wrap">
					<ul class="pagination">
						@if($pagination['current_page'] > 1)
							<li><a href="{{ URL::asset('/admin/pages/?page=1&sort_by='.$pagination['sort_by'].'&dir='.$pagination['dir']) }}">&laquo;</a></li>
							<li><a href="{{ URL::asset('/admin/pages/?page='.($pagination['current_page'] -1).'&sort_by='.$pagination['sort_by'].'&dir='.$pagination['dir']) }}">&lsaquo;</a></li>
						@endif
						@for($i=1; $i<=$pagination['last_page']; $i++)
							<li><a href="{{ URL::asset('/admin/pages/?page='.$i.'&sort_by='.$pagination['sort_by'].'&dir='.$pagination['dir']) }}" @if($pagination['current_page'] == $i) class="active" @endif>{{$i}}</a></li>
						@endfor
						@if($pagination['current_page'] < $pagination['last_page'])
							<li><a href="{{ URL::asset('/admin/pages/?page='.($pagination['current_page'] +1).'&sort_by='.$pagination['sort_by'].'&dir='.$pagination['dir']) }}">&rsaquo;</a></li>
							<li><a href="{{ URL::asset('/admin/pages/?page='.$pagination['last_page'].'&sort_by='.$pagination['sort_by'].'&dir='.$pagination['dir']) }}">&raquo;</a></li>
						@endif
					</ul>
				</div>
			@endif
			<div class="categories-list-wrap">
				...
				<table class="item-list col_1">
						...
						<tr>
							...
							<th>Ссылка
								<div class="direction" id="slug">
									<a href="{{ URL::asset('/admin/pages/?page='.$pagination['current_page'].'&sort_by=slug&dir=asc') }}" class=asc>&#9650;</a>
									<a href="{{ URL::asset('/admin/pages/?page='.$pagination['current_page'].'&sort_by=slug&dir=desc') }}" class=desc>&#9660;</a>
								</div>
							</th>
							<th>Превью</th>
							<th>Автор
								<div class="direction" id="author">
									<a href="{{ URL::asset('/admin/pages/?page='.$pagination['current_page'].'&sort_by=author&dir=asc') }}" class=asc>&#9650;</a>
									<a href="{{ URL::asset('/admin/pages/?page='.$pagination['current_page'].'&sort_by=author&dir=desc') }}" class=desc>&#9660;</a>
								</div>
							</th>
							<th>Просмотров
								<div class="direction" id="views">
									<a href="{{ URL::asset('/admin/pages/?page='.$pagination['current_page'].'&sort_by=views&dir=asc') }}" class=asc>&#9650;</a>
									<a href="{{ URL::asset('/admin/pages/?page='.$pagination['current_page'].'&sort_by=views&dir=desc') }}" class=desc>&#9660;</a>
								</div>
							</th>
							<th>Шаблон
							...
							<th>Опубликован
								<div class="direction" id="published">
									<a href="{{ URL::asset('/admin/pages/?page='.$pagination['current_page'].'&sort_by=published&dir=asc') }}" class=asc>&#9650;</a>
									<a href="{{ URL::asset('/admin/pages/?page='.$pagination['current_page'].'&sort_by=published&dir=desc') }}" class=desc>&#9660;</a>
								</div>
							</th>
							<th>Создан
					...
					@foreach($pages as $item)
						<tr data-id="{{ $item['id'] }}">
							<td>
								<a class="block-button edit" href="{{ URL::asset('/admin/pages/edit/'.$item['id']) }}" title="Редактировать">
