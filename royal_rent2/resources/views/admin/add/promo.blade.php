	...
	<div class="main-block" data-module="{{ $fields['id'] }}">
		...
		<div class="center-wrap col_1">
			...
			<div class="work-place-wrap">
				...
				<div>
					<fieldset>
						...
						<div class="row-wrap">
							<label class="fieldset-label-wrap">
								<input name="discountValue" type="number" class="text-input col_1_2" placeholder="Скидка&hellip;" @if(isset($content['discount'])) value="{{$content['discount']['value']}}" @endif>
								<span>Скидка</span>
							</label>
						</div>
						<div class="row-wrap" style="display: none">
							<label class="fieldset-label-wrap">
								<input name="discountType" type="radio" class="chbox-input" value="percent" @if(isset($content['discount'])){{ $content['discount']['percent'] }}@else checked="checked" @endif>
								<span>Процентное значение (Цена - (Цена * Скидка));</span>
							</label>
							<label class="fieldset-label-wrap">
								<input name="discountType" type="radio" class="chbox-input" value="minus" @if(isset($content['discount'])){{ $content['discount']['minus'] }}@endif>
								<span>Фиксированое значение (Цена - Скидка);</span>
							</label>
						</div>
					</fieldset>
				</div>
			...
			<div class="button-wrap tac">
				<button name="savePromo" class="control-button" type="button">Применить</button>
			</div>
		</div>
	</div>
@stop