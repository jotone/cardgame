							...
							foreach($magic['actions'] as $action_iter => $action){
								if($action['caption'] == 'terrify'){
									if(!in_array($users_data[$opponent_player]['current_deck'], $action['fear_enemyRace'])){
										foreach($action['fear_ActionRow'] as $action_row){
											$field_status[$opponent_player][$action_row]['debuffs'][] = 'terrify';
											foreach($battle_field[$opponent_player][$action_row]['warrior'] as $card_iter => $card_data){
												...
												if($allow_fear){
													...
													if( (isset($step_status['played_magic'])) && (!empty($step_status['played_magic'])) ){
														$step_status['actions']['cards'][$opponent_player][$action_row][$card_iter] = [
															'card'		=> $card['caption'],
															'strength'	=> $card_data['strength'],
															'strModif'	=> $strength,
															'operation'	=> '-'
													...
													if(isset($fury_cards[$opponent_player][$action_row][$card_iter])){
														$fury_cards[$opponent_player][$action_row][$card_iter]['strength'] = $fury_cards[$opponent_player][$action_row][$card_iter]['strModif'];
														$fury_cards[$opponent_player][$action_row][$card_iter]['strModif'] = $strength;
													}

													$battle_field[$opponent_player][$action_row]['warrior'][$card_iter]['strength'] = $strength;
													$field_status[$opponent_player][$action_row]['warrior'][$card_iter]['strengthModified'] = $strength;

													$cards_strength[$opponent_player][$action_row][$card_iter] = $strength;

													if(isset($step_status['added_cards'][$opponent_player][$action_row]) && (!in_array('spy', $played_card_actions))){
														foreach($step_status['added_cards'][$opponent_player][$action_row] as $i => $added_card){
															if(Crypt::decrypt($added_card['id']) == $battle_field[$opponent_player][$action_row]['warrior'][$card_iter]['id']){
																$step_status['added_cards'][$opponent_player][$action_row][$i]['strength'] = $battle_field[$opponent_player][$action_row]['warrior'][$card_iter]['strength'];
															}
														}
													}

													$field_status[$opponent_player][$action_row]['warrior'][$card_iter]['debuffs'][] = 'terrify';
													$field_status[$opponent_player][$action_row]['warrior'][$card_iter]['debuffs'] = array_values(array_unique($field_status[$opponent_player][$action_row]['warrior'][$card_iter]['debuffs']));
												}
											}
											$field_status[$opponent_player][$action_row]['debuffs'] = array_values(array_unique($field_status[$opponent_player][$action_row]['debuffs']));
										}
									}
								}
							}
